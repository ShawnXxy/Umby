...
public final class WeatherUtils {
    ...
    // Convert from C to F
    private static double celsToFa(double temperatureInC) {
    ...
    public static String formatHighLow(Context c, double high, double low) {
        ...
        String formattedHigh = formatTemperature(c, roundedHigh);
        String formattedLow = formatTemperature(c, roundedLow);

        String highLow = formattedHigh + " / " + formattedLow;
        return highLow;
    }

    // format wind showing speed and direction
    // Degrees as measured on a compass, NOT temperature degrees(https://www.mathsisfun.com/geometry/degrees.html)
    public static String getFormattedWind(Context c, float speed, float degrees) {
        int wind = R.string.wind_in_mph;

        if (!Location.isMetric(c)) {
            wind = R.string.wind_in_kph;
            speed = .621371192237334f * speed;
        }

        // Degrees defined at https://www.mathsisfun.com/geometry/degrees.html
        ...
        return String. format(c.getString(wind), speed, direction);
    }

    // get weather condition based on OpenWeatherMap API:https://openweathermap.org/weather-conditions
    public static String getWeatherCondition(Context c, int weatherId) {
        int stringId;
